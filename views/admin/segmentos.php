...
<head>
    ...
    <title>Gestión de Segmentos - Atlantic City</title>
...
<body>
    ...
    <div class="main-content">
        <div class="container-fluid">
            <h2 class="mb-4 fw-bold text-dark">Mantenimiento de Segmentos</h2>
            
            <div class="card border-0 shadow-sm rounded-4">
                <div class="card-header card-header-gold rounded-top-4 d-flex justify-content-between align-items-center">
                    <span><i class="fas fa-layer-group me-2"></i> Listado de Segmentos</span>
                    <button class="btn btn-dark btn-sm" data-bs-toggle="modal" data-bs-target="#segmentoModal" onclick="resetForm()">
                        <i class="fas fa-plus"></i> Nuevo Segmento
                    </button>
                </div>
                <div class="card-body">
                    <div class="table-responsive">
                        <table id="tablaSegmentos" class="table table-hover align-middle">
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header bg-dark text-white">
                    <h5 class="modal-title" id="modalTitle">Nuevo Segmento</h5>
                ...
                <div class="modal-body">
                    <form id="segmentoForm">
                        ...
                        <div class="mb-3">
                            <label class="form-label">Criterios Mínimos</label>
                            <textarea class="form-control" id="criterios_minimos" name="criterios_minimos" rows="2"></textarea>
                        </div>
                    </form>
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Cancelar</button>
                    <button type="button" class="btn btn-gold" onclick="guardarSegmento()">Guardar</button>
                </div>
            </div>
        </div>
    </div>
    ...
    <script>
        ...
        function cargarSegmentos() {
            $('#tablaSegmentos').DataTable({
                ajax: {
                    url: '../../php/segmento_controller.php',
                    type: 'POST',
                    data: { action: 'list' }
                },
                columns: [
                    ...
                    { data: 'nombre' },
                    { data: 'descripcion' },
                    { data: 'criterios_minimos' },
                    {
                        data: null,
                        render: function(data, type, row) {
                            return `
                                <button class="btn btn-sm btn-warning" onclick="editarSegmento(${row.segmento_id})"><i class="fas fa-edit"></i></button>
                                <button class="btn btn-sm btn-danger" onclick="eliminarSegmento(${row.segmento_id})"><i class="fas fa-trash"></i></button>
                            `;
                        }
                    }
                ],
        ...
        function resetForm() {
            $('#segmentoForm')[0].reset();
            $('#segmento_id').val('');
            $('#action').val('create');
            $('#modalTitle').text('Nuevo Segmento');
        }

        function guardarSegmento() {
            const formData = new FormData(document.getElementById('segmentoForm'));
            
            fetch('../../php/segmento_controller.php', {
                method: 'POST',
                body: formData
            })
        ...
        function editarSegmento(id) {
            ...
            .then(data => {
                ...
                $('#criterios_minimos').val(data.criterios_minimos);
                $('#action').val('update');
                $('#modalTitle').text('Editar Segmento');
